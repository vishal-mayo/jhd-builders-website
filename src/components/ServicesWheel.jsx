  ...
  return (
    <div className="flex justify-center items-center w-full">
      ...
      <div className="md:hidden">
        <ScaledWheel scale={0.6} selected={selected} onSelect={onSelect} />
      </div>
      
      {/* Tablet: 75% scale */}
      <div className="hidden md:block lg:hidden">
        <ScaledWheel scale={0.75} selected={selected} onSelect={onSelect} />
      </div>
      
      {/* Desktop: Full size */}
      <div className="hidden lg:block">
        <ScaledWheel scale={1} selected={selected} onSelect={onSelect} />
      </div>
    </div>
  )
}

function ScaledWheel({ scale, selected, onSelect }) {
  const scaledWidth = 560 * scale
  const scaledHeight = 560 * scale
